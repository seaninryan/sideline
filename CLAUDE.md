...
**Versioning:** `APP_VERSION` (in `lib/constants.ts`) is shown in the footer at the bottom of the app (`Here We Go · vN`). Bump it on every change that will be deployed, and tell the user which version to look for. Current: **v70**.
...
- **Storage:** a `matches` table, row per match. Columns: `id uuid pk`, `owner uuid (default auth.uid())`, `is_public bool default false`, `listed bool default true`, `short_code text unique`, `name_display text default 'full'`, `match_date timestamptz`, `my_team text`, `opponent text`, `sport text`, `data jsonb`, `updated_at timestamptz`. `data jsonb` holds the full match record and is the source of truth; the promoted columns (`match_date`, `my_team`, `opponent`, `sport`, `name_display`) are derived on every `store.set`. RLS: `own_all` policy (`owner = auth.uid()`) + `public_read` policy (`is_public = true`). The public page and OG route read only `is_public=true` rows and apply `name_display` redaction.
- **Unlisted public matches (`listed`).** A public match is listed in the global "Recent public matches" feed by default; the owner can toggle it off in `ShareSheet` (link still opens it, but it won't appear in the feed for anyone). The Landing feed query filters `.eq("is_public", true).eq("listed", true)`; `store.set` does **not** touch `listed` (managed by `ShareSheet` only, like `is_public`/`short_code`). **Schema migration (run once in Supabase):** `alter table matches add column if not exists listed boolean not null default true;` (the `default true` backfills existing rows; without the column the feed query errors and returns empty).
...
- **Team squads + listed.** A team's `squad` (string) is part of identity `(sport, name, squad)` with unique index `teams_owner_sport_name_squad_key`; blank `""` = plain club team. `listed` (bool, default `true`) mirrors `matches.listed` for filtering the public-teams feed. Migration in `docs/teams-squad-migration.sql`. Matches snapshot `usSquad`/`oppSquad` from the linked teams at link time.
...
- `buildInfographicSVG(model)` (`lib/infographic.ts`) builds a portrait (~420px wide) SVG poster that **mirrors the on-screen public-page components** (v65 re-layout): a brand banner (`brandPillSVG` + wordmark + chant); a score header using **jersey shirts** (the same silhouette as `<Jersey>`) with a neutral *Leading by N / Won by N / Tie* result chip under the leader; boxed 2×2 stats (like `StatGrid`); a chart that matches `<ScoreChart>` (darkened lines via `chartColor`, ⚽/green-umpire-flag goal markers, end-point score labels, a minute axis, sub/card rail markers from `chartMarkers`); a **combined scorers leaderboard table** (jersey + Goals/Points/Frees/Total, like `<Scorers>`); jersey lineup pitches for both teams (+ bench chips, sub arrows, card chips, corners in the timeline); and a brand footer. Keep it visually in step with the on-screen components when either changes.
- **Shared lineup-icon helper.** The per-player lineup badges — sub arrows (▲▼), card chips, own-goal mark, and goal/point tally — are computed by **one pure side-aware helper `lineupBadges(model, side, num)`** (`lib/lineup-badges.ts`, reading only `timeline` + `usScorers`/`themScorers`). It is consumed by all three lineup renderers — the editor (`MatchTracker` `subArrows`/`playerMarks`/`scoreFor`), the public page (`PublicMatch` `badges`/`scoreFor`), and the poster (`buildInfographicSVG` `drawPitch`/`drawBench`) — for **both** teams. Before this (v70) each renderer hand-rolled its own badge logic and they had drifted (home-only icons; the away team showed nothing on the editor + poster). Keep all three calling the helper rather than re-deriving badges.
- **`ShareImageModal`** (`components/ShareImageModal.tsx`) is the **shared** share-as-image panel: given a `model`, it builds the SVG, rasterises to PNG, shows the preview + a Save/Share button (Web Share, else download) with an SVG fallback. Used by **both** the editor (`MatchTracker`, via `shareModel` state) and `PublicMatch` (via `imgOpen` state) so the export experience is identical.
